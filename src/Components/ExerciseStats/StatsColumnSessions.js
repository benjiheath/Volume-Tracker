...
const SessionColumnStats = ({ exercises, exerciseM, idx, sesh, seshIdx, setExercises, repChanged }) => {
  ...
  const setTotalReps = () => {
    ...
    if (seshIdx > 0) {
      ...
      const changePercentage =
        exerciseM.sessions[seshIdx].change / exerciseM.sessions[seshIdx - 1].totalVolume;
  ...
  return (
    <div className="column-stats">
      ...
      <div className="col col-stats-t-r">{exerciseM.sessions[seshIdx].weight && totalVolume}</div>
      <div
        className={
      ...
      >
        {exerciseM.sessions[seshIdx].weight && exerciseM.sessions[seshIdx].change}
      </div>
      <div
        className={
      ...
      >
        {seshIdx > 0 &&
          ...
          `${(exerciseM.sessions[seshIdx].changePercentage * 100).toFixed(1)} %`}
      </div>
      <div
        className={
      ...
      >
        {seshIdx > 1 &&
          exerciseM.sessions[seshIdx].weight &&
          `${(exerciseM.sessions[seshIdx].totalChangePercentage * 100).toFixed(1)} %`}
      </div>
    </div>
  );
};
