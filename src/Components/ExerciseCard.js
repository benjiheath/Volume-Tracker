    ...
    <div className="exercise-card">
      <div className="card-inc-btns-container">
        ...
        <div className="plus-container">
          <Button className="btn-plus-session" text="+" onClick={addNewSession} />
        </div>
      </div>
      <ExerciseStats
        exercises={exercises}
        ...
        repChanged={repChanged}
      />
      <Button className="btn btn-delete-exercise" text="delete" onClick={deleteExercise} />
      {exerciseM.sessions.length > 1 && (
        <div className="chart-container">
          <Chart scale={{ value: { min: 0 } }} padding={[10, 20, 50, 40]} autoFit height={160} data={data2}>
            <Line
              shape=""
            ...
            />
            <Tooltip hideMarkers={true} follow={false} style={{ opacity: 0 }} />
          </Chart>
        </div>
      )}
    </div>
