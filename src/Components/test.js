  ...
  const sessionStats = exercise.sessions.map((session, idx) => {
    ...
    if (session.id === 1) {
      ...
      const changeSinceLast =
        totalVolume -
        exercise.sessions[id - 1].sets.reduce((s, accu) => s + accu) * exercise.sessions[id - 1].weight;
    ...
    if (session.id >= 2) {
      ...
      const changeSinceStart = (
        totalVolume /
          (exercise.sessions[0].sets.reduce((s, accu) => s + accu) * exercise.sessions[0].weight) -
        1
      ).toFixed(2);
