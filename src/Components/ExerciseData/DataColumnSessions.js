  ...
  return (
    <div className={exerciseM.numSets.length <= 3 ? "column" : `column-${exerciseM.numSets.length}`}>
      ...
      >
        ...
        {seshIdx + 1 === exerciseM.sessions.length && seshIdx > 0 && (
          <Button text="-" className="btn-del-sesh" onClick={deleteSession} />
        )}
      </div>
      <input
        className="input"
      ...
      {exerciseM.numSets.map((set, setIdx) => (
        <input
          ...
          className={
            setIdx + 1 === exerciseM.numSets.length && seshIdx + 1 === exerciseM.sessions.length
          ...
          key={setIdx}
          value={
            exerciseM.sessions[seshIdx].reps[setIdx] === 0 ? "" : exerciseM.sessions[seshIdx].reps[setIdx]
          }
        ></input>
      ))}
    </div>
